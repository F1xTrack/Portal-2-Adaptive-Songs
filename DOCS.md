...
- Тема карты соответствует теме приложения. Провайдер карты настраивается (OSM/Google/Yandex); для OSM доступен выбор источника тайлов. Для Google/Yandex предусмотрен fallback в браузер с zoom и toast.
...
- Длинные версии треков создаются для всех вариаций и управляются из UI; массовое удаление поддерживается.
